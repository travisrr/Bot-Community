...
really.bot itself is an Astro app on **Cloudflare Workers** (`botruns`). The same rule applies if you deploy the Next.js route to Workers: the model call is not a stamp ([Cloudflare Workers](https://developers.cloudflare.com/workers/)).
...
Built-in xAI tools (`web_search`, `x_search`, `code_interpreter`) run on xAI’s servers. Your function tools run in Next.js. Neither path installs **Gmail** or **Slack**. Those are **Grok Bot** plugins. If the finished job used the API plus a plugin Bot, say so in `connectors` — only what actually fired.
