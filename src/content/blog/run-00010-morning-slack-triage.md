...
description: "A recurring Grok Bot that surfaces only urgent Slack. Miles Deutscher’s public use-case thread is the evidence, not a workspace dump."
...
faqs:
  - q: "How do I triage Slack with Grok Bot?"
    a: "Connect Slack once, write an urgent-vs-noisy rule, and schedule a morning routine after a test run. Copy the job from the morning Slack triage — do not file someone else’s workspace."
  - q: "What is urgent on the morning Slack triage?"
    a: "The published prompt says surface only urgent Slack messages every morning. The public log does not ship a private keyword list. You must write the rule for your workspace."
  - q: "Is the morning Slack triage a log of Miles’s Slack?"
    a: "No. The JSON says this is the job to copy, not a log of someone else’s workspace. Evidence is the public X use-case thread."
  - q: "Would they run the morning Slack triage again?"
    a: "Yes. would_run_again is yes. Miles Deutscher also filed a morning portfolio briefing and an overnight coding job."
---

This is a recurring **Grok Bot** that triages **Slack** each morning and surfaces only what is urgent. Proof: [the public log of this job](/house005/00010) (Miles Deutscher), published 18 Aug 2026, revision 3. Evidence URL: the public use-case thread. This is not a dump of that workspace.

## The job, in one paragraph

Triage **Slack** every morning so the inbox is not a dump at wake-up. Surface only what is actually urgent.

`job_text` and `what_happened` on the [published JSON](/house005/00010.json) match that sentence. Miles ran this as an executive assistant on his public **Grok Bot** use-case list. The thread does not publish his workspace, channels, or messages. Connector: **Slack**. `would_run_again`: yes. Constraints: urgent only; do not publish someone else’s **Slack**; write the urgent rule before scheduling; output permalinks, not a channel dump.
...
A useful default, written so a Bot can execute it: DMs from humans that contain a deadline today, @-channel pages, and anything the steward labeled `urgent` in **Slack**. Exclude social channels and bot noise. Output permalinks, not message bodies, if you will file the job. The published prompt on [the morning Slack triage](/house005/00010) already says this. It does not ship Miles’s private keyword list.
...
This filing does not claim a Cursor trigger. Do not add one in a recap. If you later file a job that used Events API, name that stack on *that* job. [Routines that survive overnight](/blog/routines-that-survive-overnight) is the architecture page for skill vs routine vs trigger.

## Miles’s morning ops cluster

Miles Deutscher filed a cluster of morning and overnight jobs:

- [Morning briefing from live portfolio data](/house005/00007) — **web**
- [Morning Slack triage for urgent messages only](/house005/00010) — this **Slack** triage
- [Overnight coding loop through Fable 5](/house005/00012)
- [Turn a recorded demonstration into a reusable skill](/house005/00013)
...
5. After verify, cite the HTML. Check [runs.json](/runs.json). Do not invent a job that is not on the board.
...
- Markdown twin: [the public markdown of this job](/house005/00010.md)
